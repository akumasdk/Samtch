(function() {
    ...
    console.log(`[Samtch] controls_injector.js v${VERSION} starting...`);
    ...
    function injectButtons() {
        ...
        if (!document.getElementById('samtch-audio-btn')) {
            ...
            btn.onclick = (e) => {
                e.preventDefault(); e.stopPropagation();
                if (window.TwitchPlayerBridge) window.TwitchPlayerBridge.toggleAudioOnly();
            };
            rightGroup.prepend(btn);
            injectedCount++;
        }
    ...
    // Aggressive polling for the first 15 seconds (Aggressive catch hydration)
    ...
    window.samtch_controls_init_int = setInterval(() => {
        ...
        if (Date.now() - startTime > 15000) {
            ...
            window.samtch_controls_maint_int = setInterval(injectButtons, 4000);
        }
    }, 500);
