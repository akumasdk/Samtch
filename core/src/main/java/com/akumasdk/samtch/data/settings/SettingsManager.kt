...
object SettingsManager {
    ...
    private val RECENT_STREAMERS = stringPreferencesKey("recent_streamers_v2")
    ...
    fun getRecentStreamers(context: Context): Flow<List<String>> {
        return context.dataStore.data.map { preferences ->
            val json = preferences[RECENT_STREAMERS] ?: return@map emptyList()
            try {
    ...
    suspend fun addRecentStreamer(context: Context, streamer: String) {
        context.dataStore.edit { preferences ->
            val currentJson = preferences[RECENT_STREAMERS]
            val currentList = if (currentJson != null) {
                try {
            ...
            currentList.removeAll { it.equals(streamer, ignoreCase = true) }
            currentList.add(0, streamer.lowercase())

            // Keep top 20 for TV efficiency
            val limitedList = currentList.take(20)
            preferences[RECENT_STREAMERS] = Json.encodeToString(limitedList)
        }
    }
